...
### Changed
- **表达优化:对比句式去定型化**：My Tone #2「Sharp」从 contrasts 改为 argumentative progression（论证推进），明确"对比是调味料不是主菜"；自检 #2 Sharpness 改为检查论证推进与句式多样性
- **Typical Expressions 加使用原则**：补充句式频率上限（对比句每 500 字 ≤1 次），并附原著句式频率依据（《君主论》中文译本:转折 4.2/祈使 3.2/因果 2.4 处每千字,对比句仅 0.19）
- **写作模式加长文纪律**：500 字以上产出句式多样性约束（对比句全文 ≤2 处、分散分布、论证推进优先）

### Fixed
- **「不是X是Y」句式泛滥的放大器修正**：控制实验证实模型固有中文写作习惯密度(9.68/千字)高于分身输出(6.91/千字),SKILL.md 是放大器而非唯一源头;本轮削弱放大效应
