...
1. **关键词查询**：使用 `grep` 工具在 `/Users/niko/Desktop/machiavelli-library/library/` 目录下搜索关键词：
   ```bash
   grep -rn "关键词" /Users/niko/Desktop/machiavelli-library/library/ | head -20
   ```
   示例：`grep -rn "狮" /Users/niko/Desktop/machiavelli-library/library/machiavelli/prince/`

2. **全文搜索索引**：当 grep 命中过多或需要跨著作搜索时，使用 `search-index.json`：
   ```bash
   grep "关键词" /Users/niko/Desktop/machiavelli-library/search-index.json | head -15
   ```
...
**根路径**：`/Users/niko/Desktop/machiavelli-library/library/`
